...
library LibItems {
    ...
    uint256 internal constant ITEM_CATEGORY_COMPANY = 3;


    uint8 internal constant WEARABLE_SLOTS_TOTAL = 12;
    ...
    function removeFromParent(
        ...
        if (_fromContract == address(this) && bal == 1) {
            Aavegotchi storage aavegotchi = s.pyranihas[_fromTokenId];
            if (
    ...
    function checkWearableIsEquipped(uint256 _fromTokenId, uint256 _id) internal view {
        ...
        for (uint256 i; i < EQUIPPED_WEARABLE_SLOTS; i++) {
            require(s.pyranihas[_fromTokenId].equippedWearables[i] != _id, "Items: Cannot transfer wearable that is equipped");
        }
    }
}
