...
contract PyranihaFacet {
  ...
  function balanceOf(address _owner) external view returns (uint256 balance_) {
      require(_owner != address(0), "PyranihaFacet: _owner can't be address(0");
      balance_ = s.ownerTokenIds[_owner].length;
  }

  ///@notice Retrieve all details about a specific Pyra by ID
    ///@param _tokenId the identifier of the NFT to query
    ///@return aavegotchiInfo_ a struct containing all details about
  function getPyraniha(uint256 _tokenId) external view returns(PyranihaInfo memory pyranihaInfo_) {
    pyranihaInfo_ = LibPyraniha.getPyraniha(_tokenId);
  }

    ///@notice returns the time an NFT was claimed
    ///@dev will return 0 if the NFT is still an unclaimed egg
    ///@param _tokenId the identifier of the NFT
    ///@return claimTime_ the time the NFT was claimed
  function pyranihaClaimTime(uint256 _tokenId) external view returns(uint256 claimTime_) {
    claimeTime_ = s.pyranihas[_tokenId].claimTime;
  }
    ...
    function tokenByIndex(uint256 _index) external view returns (uint256 tokenId_) {
        require(_index < s.tokenIds.length, "PyranihaFacet: index beyond supply");
    ...
    function tokenOfOwnerByIndex(address _owner, uint256 _index) external view returns (uint256 tokenId_) {
        require(_index < s.ownerTokenIds[_owner].length, "PyranihaFacet: index beyond owner balance");
    ...
    function ownerOf(uint256 _tokenId) external view returns (address owner_) {
        owner_ = s.pyranihas[_tokenId].owner;
        require(owner_ != address(0), "PyranihaFacet: invalid _tokenId");
    }
    ...
    function internalTransferFrom(
        ...
        require(_to != address(0), "PyranihaFacet: Can't transfer to 0 address");
        require(_from != address(0), "PyranihaFacet: _from can't be 0 address");
        require(_from == s.pyranihas[_tokenId].owner, "PyranihaFacet: _from is not owner, transfer failed");
        require(
            _sender == _from || s.operators[_from][_sender] || _sender == s.approved[_tokenId],
            "PyranihaFacet: Not owner or approved to transfer"
        );
        LibPyraniha.transfer(_from, _to, _tokenId);
    ...
    function approve(address _approved, uint256 _tokenId) external {
        address owner = s.pyranihas[_tokenId].owner;
    ...
    function tokenURI(uint256 _tokenId) external pure returns (string memory) {
        return LibStrings.strWithUint("https://app.pyraniha.com/metadata/pyranihas/", _tokenId); //Here is your URL!
    }
