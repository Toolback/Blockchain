   ...
   |__________________________________*/
    ...
    function openEggs(uint256[] calldata _tokenIds) external {
        ...
        for (uint256 i; i < _tokenIds.length; i++) {
            ...
            require(s.pyranihas[tokenId].status == LibPyraniha.STATUS_CLOSED_EGG, "PyranihaFacet: Portal is not closed");
            require(owner == s.pyranihas[tokenId].owner, "PyranihaFacet: Only aavegotchi owner can open a portal");
            require(s.pyranihas[tokenId].locked == false, "PyranihaFacet: Can't open portal when it is locked");
    ...
    function drawRandomNumber(uint256 _tokenId) internal {
        s.pyranihas[_tokenId].status = LibPyraniha.STATUS_VRF_PENDING;
        uint256 fee = s.fee;
    ...
    function tempFulfillRandomness(bytes32 _requestId, uint256 _randomNumber) internal {
        ...
        require(s.pyranihas[tokenId].status == LibPyraniha.STATUS_VRF_PENDING, "VrfFacet: VRF is not pending");
        s.pyranihas[tokenId].status = LibPyraniha.STATUS_OPEN_PORTAL;
