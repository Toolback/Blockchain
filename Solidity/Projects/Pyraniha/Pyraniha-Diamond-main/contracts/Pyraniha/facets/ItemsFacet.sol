...
import {LibAppStorage, Modifiers, ItemType, Pyraniha, ItemType, WearableSet, NUMERIC_TRAITS_NUM, EQUIPPED_WEARABLE_SLOTS, EGG_PYRANIHAS_NUM} from "../libraries/LibAppStorage.sol";
...
contract ItemsFacet is Modifiers {
   ...
   |__________________________________*/
    ...
    function equippedWearables(uint256 _tokenId) external view returns (uint16[EQUIPPED_WEARABLE_SLOTS] memory wearableIds_) {
        wearableIds_ = s.pyranihas[_tokenId].equippedWearables;
    }
   ...
   |__________________________________*/
    ...
    function equipWearables(uint256 _tokenId, uint16[EQUIPPED_WEARABLE_SLOTS] calldata _wearablesToEquip)
        external
        onlyPyranihaOwner(_tokenId)
        onlyUnlocked(_tokenId)
    {
        Pyraniha storage pyraniha = s.pyranihas[_tokenId];
        require(pyraniha.status == LibPyraniha.STATUS_PYRANIHA, "LibPyraniha: Only valid for AG");
        emit EquipWearables(_tokenId, pyraniha.equippedWearables, _wearablesToEquip);

        address sender = LibMeta.msgSender();

        for (uint256 slot; slot < EQUIPPED_WEARABLE_SLOTS; slot++) {
            uint256 toEquipId = _wearablesToEquip[slot];
            uint256 existingEquippedWearableId = pyraniha.equippedWearables[slot];
            ...
            pyraniha.equippedWearables[slot] = uint16(toEquipId);
            ...
            if (toEquipId != 0) {
                ...
                require(LibPyraniha.pyranihaLevel(pyraniha.experience) >= itemType.minLevel, "ItemsFacet: AG level lower than minLevel");
                ...
                require(itemType.slotPositions[slot] == true, "ItemsFacet: Wearable can't be equipped in slot");
                {
                    ...
                    if (allowedCollaterals.length > 0) {
                        uint256 collateralIndex = s.collateralTypeIndexes[pyraniha.collateralType];
    ...
    function useConsumables(
        ...
        require(s.pyranihas[_tokenId].status == LibPyraniha.STATUS_PYRANIHA, "LibPyraniha: Only valid for claimed Pyraniha ");
        ...
        for (uint256 i; i < _itemIds.length; i++) {
            ...
            if (itemType.kinshipBonus > 0) {
                uint256 kinship = (uint256(int256(itemType.kinshipBonus)) * quantity) + s.pyranihas[_tokenId].interactionCount;
                s.pyranihas[_tokenId].interactionCount = kinship;
            } else if (itemType.kinshipBonus < 0) {
                uint256 kinshipBonus = LibAppStorage.abs(itemType.kinshipBonus) * quantity;
                if (s.pyranihas[_tokenId].interactionCount > kinshipBonus) {
                    s.pyranihas[_tokenId].interactionCount -= kinshipBonus;
                } else {
                    s.pyranihas[_tokenId].interactionCount = 0;
                }
            }

            {
                ...
                if (boost) {
                    s.pyranihas[_tokenId].lastTemporaryBoost = uint40(block.timestamp);
                    s.pyranihas[_tokenId].temporaryTraitBoosts = itemType.traitModifiers;
                }
            }

            //Increase experience
            if (itemType.experienceBonus > 0) {
                uint256 experience = (uint256(itemType.experienceBonus) * quantity) + s.pyranihas[_tokenId].experience;
                s.pyranihas[_tokenId].experience = experience;
            }
